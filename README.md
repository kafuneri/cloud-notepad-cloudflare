...
* 访问站点下的 `/list` 路径，可总览当前存储在 **KV 数据库** 中的所有笔记超链接及**最后修改时间**。为了保障隐私安全，该全局路由已接入 **HTTP Basic Auth** 基础鉴权机制：
* 访问时浏览器会原生弹出安全登录框，**用户名**可任意填写，**密码**需精确输入你在环境变量中配置的 **`LIST_SECRET`** 的值。
* 若后台未配置 **`LIST_SECRET`** 环境变量，系统将出于安全保护默认阻断访问，并返回 **403 Forbidden** 状态码。
...
* 变量名称：**`LIST_SECRET`**，值：输入/list页面的访问密码。
